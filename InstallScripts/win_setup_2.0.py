...
def download_file(url: str, dest_path: Path, logger):
...
def setup_mysql(base_dir: Path, software_dir: Path, logger, app):
    ...
    if not mysql_dir.exists():
        ...
        download_file(MYSQL_URL, zip_path, logger)
...
def setup_local_python(software_dir: Path, logger) -> Path:
    ...
    download_file(PYTHON_URL, installer_path, logger)
...
def setup_morpheus(software_dir: Path, logger):
    ...
    if not morpheus_dir.exists():
        ...
        download_file(MORPHEUS_URL, zip_path, logger)
        extract_archive(zip_path, software_dir, logger)
...
def setup_msfragger(software_dir: Path, logger):
    ...
    if not msfrag_dir.exists():
        ...
        messagebox.showinfo("Action Required", f"Please select release 4.4.1 and fill in the form. A zip file will be sent to your email. \n\n Add the zip to {software_dir}\n\nPress OK when complete.")
        logger.info("Manual download confirmed.")
...
def setup_msbooster(software_dir: Path, logger):
    ...
    if not msboost_dir.exists():
        ...
        download_file(MSBOOSTER_URL, jar_path, logger)  
        
def setup_percolator(software_dir: Path, logger):
    ...
    if not percolator_dir.exists(): 
        ...
        download_file(PERCOLATOR_URL, exe_path, logger)

def setup_philosopher(software_dir: Path, logger):
    ...
    if not phil_dir.exists():
        ...
        download_file(PHILOSOPHER_URL, zip_path, logger)
...
class MaSpeQCSetupApp(ctk.CTk):
    ...
    def run_setup_tasks(self):
        ...
        try:
            setup_mysql(base_dir, software_dir, self.logger, self)
            status['MySQL'] = True
        ...
        try:
            setup_nodejs(base_dir, software_dir, self.logger)
            status['Node.js'] = True
        except Exception as e:
        ...
        try:
            local_python = setup_local_python(software_dir, self.logger)
        ...
        try:
            setup_mzmine(software_dir, self.logger)
            status['MZmine'] = True
        except Exception as e:
            status['MZmine'] = False
        ...
        try:
            setup_morpheus(software_dir, self.logger)
        ...
        try:
            setup_msbooster(software_dir, self.logger)
        ...
        try:
            setup_percolator(software_dir, self.logger)
        ...
        try:
            setup_philosopher(software_dir, self.logger)
